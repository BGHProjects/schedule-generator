...
export function validateGameMove(
  ...
  if (conflictingGames.length > 0) {
    ...
    let conflictingTeam = "";
    if (
      conflictGame.team1Id === game.team1Id ||
      conflictGame.team2Id === game.team1Id
    ) {
  ...
  const gameAtDestination = schedule.find(
    (g) =>
      g.id !== game.id && g.court === newCourt && g.timeSlot === newTimeSlot
  );

  if (gameAtDestination) {
    // This is a swap scenario - validate the swap
    const swapValidation = validateSwap(
      game,
      gameAtDestination,
      schedule,
      data
  ...
  if (game.refId) {
    const refValidation = validateRefereeForMove(
      game,
      newTimeSlot,
      schedule,
      data
...
function validateSwap(
  ...
  if (gameAConflicts.length > 0) {
    ...
    let conflictingTeam = "";
    if (
      conflict.team1Id === gameB.team1Id ||
      conflict.team2Id === gameB.team1Id
    ) {
  ...
  if (courtConflictAtA) {
    const conflictTeam1 = data.teams.find(
      (t) => t.id === courtConflictAtA.team1Id
    );
    const conflictTeam2 = data.teams.find(
      (t) => t.id === courtConflictAtA.team2Id
    );
  ...
  if (gameA.refId) {
    const refValidationA = validateRefereeForMove(
      gameA,
      gameB.timeSlot,
      schedule,
      data
  ...
  if (gameB.refId) {
    const refValidationB = validateRefereeForMove(
      gameB,
      gameA.timeSlot,
      schedule,
      data
...
function validateRefereeForMove(
  ...
  if (
    ref.teamId &&
    (ref.teamId === game.team1Id || ref.teamId === game.team2Id)
  ) {
  ...
  const refBusyGame = schedule.find(
    (g) =>
      g.id !== game.id && g.refId === game.refId && g.timeSlot === newTimeSlot
  );
  ...
  if (ref.teamId) {
    const refTeamGame = schedule.find(
      (g) =>
        g.id !== game.id &&
        g.timeSlot === newTimeSlot &&
        (g.team1Id === ref.teamId || g.team2Id === ref.teamId)
    );
  ...
  switch (reason) {
    ...
    case "swap_ref_conflict_a":
      return `❌ REFEREE SWAP CONFLICT\n\nCannot swap "${
        ...
        details.gameB
      }".\n\n🔍 SPECIFIC ISSUE:\nThe first game's referee (${
        details.refName
      }) would have a conflict at the new time slot.\n\n${getRefConflictDetails(
        details
      )}`;
